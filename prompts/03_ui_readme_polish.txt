...
1. ADD DEVICE IDENTIFICATION TO INTERACTIVE MENU
   ...
   b) Implement the handler method _run_device_inventory():
      ...
      - Reuse NetWatch._print_device_id_table() — either by importing it or
      ...
      - Actually, the cleanest approach: move _print_device_id_table() to
        ui/display.py as a method on Display class, so both netwatch.py and
   ...
   c) After any scan completes in the interactive controller (when it calls
      NetWatch.perform_scan or NetWatch.run_full_assessment), the device ID
...
2. MOVE _print_device_id_table TO ui/display.py
   To avoid code duplication between netwatch.py and interactive_controller.py:
   ...
   a) Add a new method to the Display class in ui/display.py:
      def show_device_inventory(self, device_identities, eol_data=None):
          ...
          # Same logic as current _print_device_id_table in netwatch.py
          # Plus optional EOL column from prompt 01
          # Use self.console for output

   b) In netwatch.py, replace _print_device_id_table() body with:
...
3. ADD DEVICE COUNT TO SCAN SUMMARY STATS
   In netwatch.py, the calculate_stats() method returns a dict used by
...
VERIFY:
  ...
  - python3 -m py_compile netwatch.py
  - python3 netwatch.py --help (verify --identify shown)
...
FILES TO READ FIRST:
  ...
  - netwatch.py (lines 1104-1138 for _print_device_id_table, lines 527-540 for
